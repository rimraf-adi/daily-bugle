...
if active_module == "1. arXiv Research Ingestion":
    ...
    with col1:
        query_type = st.radio("Query Mode", ["Category Code", "Free-text Search"], horizontal=True)
        if query_type == "Category Code":
            # Select subject
            subject = st.selectbox("Subject Area", list(SUBJECT_TAXONOMY.keys()), index=9)  # Default: Computer Science
            sub_cats = SUBJECT_TAXONOMY[subject]
            selected_cat = st.selectbox("Category", list(sub_cats.keys()), format_func=lambda c: f"{c} - {sub_cats[c]}")
            search_query = f"cat:{selected_cat}"
        else:
            search_query = st.text_input("Custom Search Query", value="diffusion models AND robotics")

    with col2:
        max_results = st.number_input("Max Results", min_value=1, max_value=25, value=5)
        sort_by = st.selectbox("Sort By", ["submittedDate", "lastUpdatedDate", "relevance"])

    with col3:
        sort_order = st.selectbox("Sort Order", ["descending", "ascending"])
        fetch_btn = st.button("🚀 Fetch Raw arXiv Data", use_container_width=True, type="primary")

    if fetch_btn or st.session_state.get("arxiv_cached_data"):
        if fetch_btn:
            with st.spinner("Fetching raw Atom feed from arXiv API..."):
                t0 = time.time()
                client = ArxivClient(timeout=20)
                # Fetch raw content by building URL
        ...
        if cache:
            papers = cache["papers"]
            st.info(f"Retrieved **{len(papers)} papers** in `{cache['latency']:.2f}s` | Query URL: `{cache['url']}`")
...
# ==============================================================================
    ...
    reddit_mode = st.radio("Mode", ["Ad-Hoc Subreddit Explorer", "Subreddit Watchlist Tracker"], horizontal=True)

    if reddit_mode == "Ad-Hoc Subreddit Explorer":
        col1, col2, col3 = st.columns([2, 1, 1])
        with col1:
            sub_input = st.text_input("Subreddit Name", value="LocalLLaMA")
        with col2:
            listing = st.selectbox("Listing Stream", ["hot", "new", "top", "rising"])
            time_filter = st.selectbox("Time Filter (for top)", ["day", "week", "month", "year", "all"])
        with col3:
            r_limit = st.number_input("Limit", min_value=1, max_value=25, value=5)
            r_fetch = st.button("📡 Fetch Raw Reddit Feed", type="primary", use_container_width=True)

        if r_fetch or st.session_state.get("reddit_cached_data"):
            if r_fetch:
        ...
        with col_wl2:
            ...
            new_sub_name = st.text_input("Subreddit", placeholder="e.g. singularity")
            new_sub_cat = st.text_input("Category", placeholder="e.g. AGI & Frontier Tech")
            new_sub_stream = st.selectbox("Stream", ["hot", "new", "top"])
        ...
        with col_poll1:
            poll_btn = st.button("🔄 Poll New Unseen Posts (Deduplication Test)", type="primary", use_container_width=True)
        with col_poll2:
            if st.button("🧹 Reset Seen History", use_container_width=True):
                tracker.reset_seen()
...
# ==============================================================================
    ...
    col1, col2, col3 = st.columns([2, 1, 1])
    with col1:
        folder = st.text_input("Mailbox Folder", value="INBOX")
        custom_query = st.text_input("Custom Search Query (optional)", placeholder='e.g. FROM "substack.com"')
    with col2:
        g_limit = st.number_input("Limit Emails", min_value=1, max_value=20, value=5)
        unread_only = st.checkbox("Unread Only (UNSEEN)", value=False)
    with col3:
        g_fetch = st.button("📨 Fetch Gmail Newsletters", type="primary", use_container_width=True)

    if g_fetch or st.session_state.get("gmail_cached_data"):
        if g_fetch:
...
# ==============================================================================
    ...
    with col1:
        ...
        if model_choice == "Custom Model ID":
            model_name = st.text_input("Custom Model ID", value="anthropic/claude-3.5-sonnet")
        else:
    ...
    with col2:
        ...
        send_btn = st.button("🚀 Send to OpenRouter", type="primary", use_container_width=True)
...
# ==============================================================================
    ...
    if st.button("⚡ Fetch Multi-Source Snapshot", type="primary"):
